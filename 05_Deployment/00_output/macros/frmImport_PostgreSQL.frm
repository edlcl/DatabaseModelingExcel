VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmImport_PostgreSQL 
   Caption         =   "Import"
   ClientHeight    =   7860
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   6060
   OleObjectBlob   =   "frmImport_PostgreSQL.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "frmImport_PostgreSQL"
...
Private Sub btnRefreshDatabase_Click()
    ...
    Set conn = basImport_PostgreSQL.CreateConnection(Trim(cboSQLServer.text), _
                            "", _
                            Trim(txtUser.text), _
                            Trim(txtPassword.text))
    conn.Open
    sSQL = basImport_PostgreSQL.GetDatabasesSQL()
    ...
    Do While Not oRs.EOF
        ...
        If oRs("name") = basImport_PostgreSQL.SQL_SERVER_DATABASE_NAME Then
            lastIndex = index
        End If
        '-- Move next record
        oRs.MoveNext
    ...
    '-- Set public variant
    basImport_PostgreSQL.SQL_SERVER_NAME = cboSQLServer.text
    basImport_PostgreSQL.SQL_USER_NAME = txtUser.text
    Exit Sub
Flag_Err:
...
Private Sub btnRefreshTable_Click()
    ...
    If Cells.item(Table_Sheet_Row_TableName, _
        ...
        sTableName = basImport_PostgreSQL.SQL_SERVER_TABLE_NAME
    Else
    ...
    Set conn = basImport_PostgreSQL.CreateConnection(Trim(cboSQLServer.text), _
                            ...
                            Trim(txtPassword.text))
    conn.Open
    sSQL = basImport_PostgreSQL.GetTablesSQL()
    ...
    '-- Set public variant
    basImport_PostgreSQL.SQL_SERVER_DATABASE_NAME = cboDatabase.text
    '-- Close record set
    oRs.Close
    conn.Close
    Set oRs = Nothing
...
Private Sub Init()
    ...
    '-- Init
    If Len(basImport_PostgreSQL.SQL_SERVER_NAME) = 0 Then
        cboSQLServer.text = "localhost"
    Else
        cboSQLServer.text = basImport_PostgreSQL.SQL_SERVER_NAME
    End If
    cboSQLServer.SetFocus
    
    If Len(basImport_PostgreSQL.SQL_USER_NAME) = 0 Then
        txtUser.text = "postgres"
    Else
        txtUser.text = basImport_PostgreSQL.SQL_USER_NAME
    End If
    ...
    For index = 0 To Me.lstTables.ListCount - 1
        If lstTables.selected(index) Then
            ...
            If isSetPublicVarient = False Then
                basImport_PostgreSQL.SQL_SERVER_TABLE_NAME = tableName
                isSetPublicVarient = True
            End If
            Set table = basImport_PostgreSQL.GetLogicalTable(conn, tableName)
            '-- Write to sheet
            shtCurrent.Select
