VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmReverse_SQLServer 
   Caption         =   "Reverse"
   ClientHeight    =   7860
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6060
   OleObjectBlob   =   "frmReverse_SQLServer.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "frmReverse_SQLServer"
...
Private Sub btnRefreshDatabase_Click()
    ...
    Set conn = basReverse_SQLServer.CreateConnection(Trim(cboSQLServer.text), _
                            "master", _
                            Trim(txtUser.text), _
    ...
    Do While Not oRs.EOF
        ...
        If oRs("name") = basReverse_SQLServer.SQL_SERVER_DATABASE_NAME Then
            lastIndex = index
        End If
        '-- Move next record
        oRs.MoveNext
    ...
    '-- Set public variant
    basReverse_SQLServer.SQL_SERVER_NAME = cboSQLServer.text
    Exit Sub
Flag_Err:
...
Private Sub btnRefreshTable_Click()
    ...
    If Cells.Item(Table_Sheet_Row_TableName, _
        ...
        sTableName = basReverse_SQLServer.SQL_SERVER_TABLE_NAME
    Else
    ...
    '-- Set public variant
    basReverse_SQLServer.SQL_SERVER_DATABASE_NAME = cboDatabase.text
    '-- Close record set
    oRs.Close
    conn.Close
    Set oRs = Nothing
...
Private Sub Init()
    ...
    '-- Init
    If Len(basReverse_SQLServer.SQL_SERVER_NAME) = 0 Then
        cboSQLServer.text = ""
    Else
        cboSQLServer.text = basReverse_SQLServer.SQL_SERVER_NAME
    End If
    cboSQLServer.SetFocus
    ...
    For index = 0 To Me.lstTables.ListCount - 1
        If lstTables.selected(index) Then
            ...
            If isSetPublicVarient = False Then
                basReverse_SQLServer.SQL_SERVER_TABLE_NAME = tableName
                isSetPublicVarient = True
            End If
            Set table = basReverse_SQLServer.GetLogicalTable(conn, tableName)
            '-- Write to sheet
            shtCurrent.Select
