VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmReverse_MySQL 
   Caption         =   "Reverse"
   ClientHeight    =   8145
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6075
   OleObjectBlob   =   "frmReverse_MySQL.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "frmReverse_MySQL"
...
Private Sub btnRefreshDatabase_Click()
    ...
    Set conn = basReverse_MySQL.CreateConnection(Trim(txtServer.text), _
                            "", _
                            Trim(txtUser.text), _
    ...
    Do While Not oRs.EOF
        ...
        If oRs("SCHEMA_NAME") = basReverse_MySQL.SERVER_DATABASE_NAME Then
            lastIndex = index
        End If
        '-- Move next record
        oRs.MoveNext
    ...
    '-- Set public variant
    basReverse_MySQL.SERVER_NAME = txtServer.text
    basReverse_MySQL.DRIVER_NAME = cboDriver.text
    basReverse_MySQL.PORT_ID = txtPort.text
    Exit Sub
Flag_Err:
...
Private Sub btnRefreshTable_Click()
    ...
    If Cells.Item(Table_Sheet_Row_TableName, _
        ...
        sTableName = basReverse_MySQL.SERVER_TABLE_NAME
    Else
    ...
    '-- Set public variant
    basReverse_MySQL.SERVER_DATABASE_NAME = cboDatabase.text
    '-- Close record set
    oRs.Close
    conn.Close
    Set oRs = Nothing
...
Private Sub Init()
    ...
    '-- Init Server
    If Len(basReverse_MySQL.DRIVER_NAME) > 0 Then
        cboDriver.text = basReverse_MySQL.DRIVER_NAME
    End If
    If Len(basReverse_MySQL.PORT_ID) > 0 Then
        txtPort.text = basReverse_MySQL.PORT_ID
    Else
        txtPort.text = "3306"
    End If
    If Len(basReverse_MySQL.SERVER_NAME) > 0 Then
        txtServer.text = basReverse_MySQL.SERVER_NAME
    Else
    ...
    For index = 0 To Me.lstTables.ListCount - 1
        If lstTables.selected(index) Then
            ...
            If isSetPublicVarient = False Then
                basReverse_MySQL.SERVER_TABLE_NAME = tableName
                isSetPublicVarient = True
            End If
            Set table = basReverse_MySQL.GetLogicalTable(conn, tableName)
            '-- Write to sheet
            shtCurrent.Select
